package ro.ciacob.maidens.generators.harmony.analyzers {
	...
	public class ChordProgression extends AbstractContentAnalyzer implements IMusicalContentAnalyzer {
		...
		override public function analyze (
			...
			// next Music Unit. This directly influences the linkeliness of held notes on a
			...
			if (prevContent.length > 0) {
				...
				// the same.
				var currNumPitches : uint = targetMusicUnit.pitches.length;
				_computeScoreLimits (currNumPitches);
				
				// Compute and store the chord progression score.
				var rawScore : int = _computeProgressionScore (prevUnit, targetMusicUnit);
			...
			// been any voice leading. Therefore, store a score of `DEFAULT_SCORE` for it.
			else {
		...
		/**
		 * Calculates the maximum and minimum posible scores for the given number of 
		 * pitches/voices.
		 * [CHECKED]
		 */
		 ...
		 * four-part writing.
		 *
		 * This is a mere approximation, done by observing the melodical relationship each
		...
		private function _computeProgressionScore (unitA : IMusicUnit, unitB : IMusicUnit) : uint {
			...
			// If this is not the case, we dupplicate the lesser chord until it has
			// the same number of pitches as the greater chord, or more. If needed, we trim
			// down the dupplicated chord, top to bottom.
